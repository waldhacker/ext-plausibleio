...
function addContainer( element, height, width ) {
  this.svg = d3.default.select( element ).append('svg')
  ...
  if (this.options.responsive) {
    d3.default.select(this.options.element).style({'position': 'relative', 'padding-bottom': (this.options.aspectRatio*100) + '%'});
    d3.default.select(this.options.element).select('svg').style({'position': 'absolute', 'width': '100%', 'height': '100%'});
    d3.default.select(this.options.element).select('svg').select('g').selectAll('path').style('vector-effect', 'non-scaling-stroke');
...
function setProjection( element, options ) {
  ...
  if ( options.scope === 'usa' ) {
    projection = d3.default.geo.albersUsa()
      .scale(width)
      .translate([width / 2, height / 2]);
  }
  else if ( options.scope === 'world' ) {
    projection = d3.default.geo[options.projection]()
...
function addStyleBlock() {
  if ( d3.default.select('.datamaps-style-block').empty() ) {
    d3.default.select('head').append('style').attr('class', 'datamaps-style-block')
  ...
  if ( options.highlightOnHover || options.popupOnHover ) {
    svg.selectAll('.datamaps-subunit')
      .on('mouseover', function(d) {
        var $this = d3.default.select(this);
      ...
      .on('mouseout', function() {
        ...
        d3.default.selectAll('.datamaps-hoverover').style('display', 'none');
      });
  }
...
function addLegend(layer, data, options) {
  ...
  var hoverover = d3.default.select( this.options.element ).append('div')
    .attr('class', 'datamaps-legend')
    .html(html);
}

  function addGraticule ( layer, options ) {
    var graticule = d3.default.geo.graticule();
    ...
    .enter()
      ...
      .attr('d', function(datum) {
          ...
          if (options.greatArc) {
            ...
            var greatArc = d3.default.geo.greatArc()
...
function Datamap( options ) {
  if ( typeof d3.default === 'undefined' || typeof topojson === 'undefined' ) {
  ...
  if ( d3.default.select( this.options.element ).select('svg').length > 0 ) {
...
Datamap.prototype.resize = function () {
  ...
  if (options.responsive) {
    var newsize = options.element.clientWidth,
        oldsize = d3.default.select( options.element).select('svg').attr('data-width');

    d3.default.select(options.element).select('svg').selectAll('g').attr('transform', 'scale(' + (newsize / oldsize) + ')');
  }
}
...
Datamap.prototype.draw = function() {
  ...
  if ( options.geographyConfig.dataUrl ) {
    d3.default.json( options.geographyConfig.dataUrl, function(error, results) {
  ...
  return this;
    ...
    function draw (data) {
      ...
      if ( self.options.dataUrl ) {
        ...
        d3.default[self.options.dataType](self.options.dataUrl, function(data) {
      ...
      if ( self.options.geographyConfig.popupOnHover || self.options.bubblesConfig.popupOnHover) {
        var hoverover = d3.default.select( self.options.element ).append('div')
...
Datamap.prototype.updatePopup = function (element, d, options) {
  ...
  element.on('mousemove', function() {
    var position = d3.default.mouse(self.options.element);
    d3.default.select(self.svg[0][0].parentNode).select('.datamaps-hoverover')
  ...
  d3.default.select(self.svg[0][0].parentNode).select('.datamaps-hoverover').style('display', 'block');
};
