...
{
    public function getGoalPropertiesData(string $goal, string $plausibleSiteId, string $timeFrame, FilterRepository $filters, array $propertyNames): array
    {
        $result = [];
    ...
    public function getGoalsData(string $plausibleSiteId, string $timeFrame, FilterRepository $filters): array
    {
        ...
        if ($goalFilterActivated && count($responseData['data']) > 0) {
            $subdata = $this->getGoalPropertiesData(
                ...
                $timeFrame,
                $filters,
                $responseData['data'][0]['props'] ?? [] // ?? -> Backwards compatibility with Plausible versions that do not yet support props retrieval
            );
            if (count($subdata) > 0) {
